...
* Rancher Charts now use `rancher/kuberlr-kubectl` instead of `rancher/kubectl`. The new image is functionally equivalent but provides greater Kubernetes version compatibility. If you need to preserve the use of `rancher/kubectl`, you can override it by modifying the chart's `.values`. See https://github.com/rancher/rancher/issues/48614[#48614].
...
* The v1alpha1 API in CAPRKE2 (cluster-api-provider-rke2) is deprecated. For more details, see https://github.com/rancher/cluster-api-provider-rke2/issues/797[rancher/cluster-api-provider-rke2#797] and https://github.com/rancher/rancher/issues/53369[#53369].

=== Major Bug Fixes

* Fixed an issue where etcd nodes in K3s and RKE2 node driver clusters could get stuck in a `Reconciling` state and fail to delete. See https://github.com/rancher/rancher/issues/38926[#38926].
