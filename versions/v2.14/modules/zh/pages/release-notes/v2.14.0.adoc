...
* Upgrading to Rancher v2.14.0 breaks Google OAuth SSO, causing login attempts to fail and redirect to `/dashboard/auth/login?err=client`. This is due to a regression involving `SSL_CERT_DIR` (when `tls: ingress` is used) and a type casing mismatch in the new `/v1-public` authentication endpoints. As a workaround, you can roll back to Rancher v2.13. See https://github.com/rancher/rancher/issues/54387[#54387].
